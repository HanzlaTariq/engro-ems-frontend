...
export default function QuarterlySpotCheckForm() {
    ...
    useEffect(() => {
        ...
        const fetchRecord = async () => {
            try {
                ...
                const res = await API.get(`/api/quarterly-spot-check/${id}`, {
                ...
                if (rec.verifiedBy === "Verified") {
                    ...
                    navigate("/quarterly-spot-check-list");
                    return;
                }
  ...
  try {
    ...
    await (isEditMode
      ? API.put(`/api/quarterly-spot-check/${id}`, form, { headers: { Authorization: `Bearer ${token}` } })
      : API.post("/api/quarterly-spot-check", form, { headers: { Authorization: `Bearer ${token}` } }));

    Swal.fire({
      icon: 'success',
      title: isEditMode ? 'Updated!' : 'Saved!',
      text: isEditMode
        ? 'Quarterly Spot Check updated successfully!'
        : 'Quarterly Spot Check saved successfully!',
      timer: 2000,
