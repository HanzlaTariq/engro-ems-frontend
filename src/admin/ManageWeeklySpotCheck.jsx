...
export default function ManageWeeklySpotCheck() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const res = await API.get("/api/spot-check", {
  ...
  const handleVerify = async (id) => {
    const confirm = await Swal.fire({
      title: 'Verify Weekly Spot Check?',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'success',
        title: 'Reverted!',
        text: 'The Weekly Spot Check record has been reverted and the warehouse manager has been notified.',
        timer: 2500,
        showConfirmButton: false,
      });
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              Manage Weekly Spot Checks
            </h1>
            <p className="text-gray-600 mt-2">
              Review and verify weekly spot check submissions
            </p>
          </div>
          <button
                  ...
                  <div className="p-5">
                    ...
                    <div className="space-y-2 mb-6">
                      <div className="flex items-center justify-between">
                        ...
                        <span className={`text-sm font-medium ${rec.elcb?.weekly?.comment ? 'text-green-600' : 'text-gray-400'}`}>
                          {rec.elcb?.weekly?.comment || "Not checked"}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6">
                ...
                {/* ELCB Section */}
                {(selectedRecord.elcb?.weekly || selectedRecord.elcb?.quarterly) && (
                  <div className="mb-6">
                    <h3 className="font-bold text-lg text-gray-800 mb-2">⚡ ELCB Check</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {selectedRecord.elcb?.weekly && (
