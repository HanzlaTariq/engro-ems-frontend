...
export default function ManageSafetyTalkTrucker() {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const res = await API.get("/api/safety-talk-trucker", {
  ...
  const handleVerify = async (id) => {
    ...
    try {
      ...
      await API.put(
        `/api/safety-talk-trucker/verify/${id}`,
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-800">
        🚚 Manage Safety Talk (Trucker) Records
      </h1>
          ...
          <table className="min-w-full text-xs text-gray-700">
            <thead className="bg-gray-200 text-gray-800 uppercase text-[10px]">
              <tr>
                ...
                <th className="py-1 px-2 border">Conducted By</th>
                <th className="py-1 px-2 border">Truck No</th>
                <th className="py-1 px-2 border">Driver Name</th>
                <th className="py-1 px-2 border">Topic</th>
                <th className="py-1 px-2 border">Location</th>
                <th className="py-1 px-2 border">Plant Code</th>
                <th className="py-1 px-2 border">Remarks</th>
              ...
              {records.length === 0 ? (
                ...
                records.map((rec) => (
                  <tr key={rec._id} className="text-center border-t hover:bg-gray-50 transition">
                    ...
                    <td className="py-1 px-2 border">{rec.conductedBy}</td>
                    <td className="py-1 px-2 border">{rec.truckNo}</td>
                    <td className="py-1 px-2 border">{rec.driverName}</td>
                    <td className="py-1 px-2 border">{rec.topic}</td>
                    <td className="py-1 px-2 border">{rec.storageLocation}</td>
